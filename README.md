...
## MVP Coverage
- **Home**: monthly spend, renewals timeline(D-7/D-14), runs today, favorites, system notes, recent activity.
- **Launch**: search + filter chips + run sheet modal + result viewer + save result as note.
- **Subscriptions**: summary, sorting, card list, status toggle, add/edit/detail.
- **Vault**: encrypted key CRUD, rotate, disable, metadata-only display(last4).
- **Logs**: filters, detail JSON copy, mini insights(most-used/fail-rate).
